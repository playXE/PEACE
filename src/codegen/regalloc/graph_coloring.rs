use super::liveness::{InterferenceGraph, Move};
use crate::codegen::*;
use crate::ir::*;
...
pub struct GraphColoring<'a> {
    ...
    precolored: LinkedHashSet<usize>,
    /// all colors available
    colors: LinkedHashMap<RegGroup, LinkedHashSet<usize>>,
    /// temporaries, not precolored and not yet processed
    initial: LinkedHashSet<usize>,

    /// list of low-degree non-move-related nodes
    worklist_simplify: LinkedHashSet<usize>,
    /// low-degree move related nodes
    worklist_freeze: LinkedHashSet<usize>,
    /// nodes marked for possible spilling during this round
    worklist_spill: LinkedHashSet<usize>,
    /// nodes that are selected for spilling, but not yet spilled
    /// (select_spill() is called on it)
    waiting_for_spill: LinkedHashSet<usize>,
    /// nodes marked for spilling during this round
    spilled_nodes: LinkedHashSet<usize>,
    /// temps that have been coalesced
    /// when u <- v is coalesced, v is added to this set and u put back on some
    /// work list
    coalesced_nodes: LinkedHashSet<usize>,
    /// nodes successfully colored
    colored_nodes: LinkedHashSet<usize>,
    ...
    movelist: LinkedHashMap<usize, LinkedHashSet<Move>>,
    /// when a move (u, v) has been coalesced, and v put in coalescedNodes,
    /// then alias(v) = u
    alias: LinkedHashMap<usize, usize>,

    // for validation use
    /// we need to log all registers get spilled with their spill location
    spill_history: LinkedHashMap<usize, Box<Node>>,
    /// we need to know the mapping between scratch temp -> original temp
    spill_scratch_temps: LinkedHashMap<usize, usize>,
}

impl<'a> GraphColoring<'a> {
    /// starts coloring
    pub fn start(cf: &'a mut LIRFunction) -> GraphColoring<'a> {
        GraphColoring::start_with_spill_history(LinkedHashMap::new(), LinkedHashMap::new(), 0, cf)
    }
    ...
    fn start_with_spill_history(
        ...
        let ig = super::liveness::build_interference_graph_chaitin_briggs(cf);
        ...
        let coloring = GraphColoring {
            ...
            colors: {
                ...
                map.insert(RegGroup::GPR, LinkedHashSet::new());
                map.insert(RegGroup::FPR, LinkedHashSet::new());
                map
            },
    ...
    fn regalloc(mut self) -> GraphColoring<'a> {
        ...
        for reg in all_regs().values() {
            let reg_id = reg.any_reg_id();
            self.precolored.insert(reg_id);
        }

        // put usable registers as available colors
        for reg in all_usable_regs().iter() {
            let reg_id = reg.any_reg_id();
            let group = pick_group_for_reg(reg_id);
    ...
    fn checkinv_degree(&self) {
        ...
        for u in union.iter() {
            ...
            let set: LinkedHashSet<usize> = {
                let adj: LinkedHashSet<usize> = self.ig.get_adj_list(*u).clone();

                let mut union_: LinkedHashSet<usize> = LinkedHashSet::new();
                add_all(&mut union_, self.precolored.clone());
                add_all(&mut union_, union.clone());

                {
                    let mut intersect: LinkedHashSet<usize> = LinkedHashSet::new();
    ...
    fn add_to_movelist(
        movelist: &mut LinkedHashMap<usize, LinkedHashSet<Move>>,
        reg: usize,
        mov: Move,
    ) {
    ...
    fn get_movelist(&self, reg: usize) -> LinkedHashSet<Move> {
    ...
    fn n_regs_for_node(&self, node: usize) -> usize {
        number_of_usable_regs_in_group(self.ig.get_group_of(node))
    }

    fn is_move_related(&mut self, node: usize) -> bool {
        !self.node_moves(node).is_empty()
    }

    fn is_spillable(&self, temp: usize) -> bool {
    ...
    fn node_moves(&mut self, node: usize) -> LinkedHashSet<Move> {
    ...
    fn adjacent(&self, n: usize) -> LinkedHashSet<usize> {
    ...
    fn decrement_degree(&mut self, n: usize) {
    ...
    fn enable_moves(&mut self, nodes: LinkedHashSet<usize>) {
    ...
    fn coalesce(&mut self) {
        ...
        // cannot coalesce
        if precolored_u {
            let group = pick_group_for_reg(u);
        ...
        if precolored_v {
            let group = pick_group_for_reg(v);
    ...
    pub fn get_alias(&self, node: usize) -> usize {
    ...
    fn check_ok(&self, u: usize, v: usize) -> bool {
    ...
    fn check_conservative(&self, u: usize, v: usize) -> bool {
    ...
    fn conservative(&self, nodes: LinkedHashSet<usize>, n_regs_for_group: usize) -> bool {
        let mut k = 0;
        for n in nodes.iter() {
    ...
    fn freeze_heuristics(&mut self) -> usize {
        use std::f32;
    ...
    fn assign_colors(&mut self) {
        ...
        let mut coloring_queue: Vec<usize> = self.coloring_queue_heuristic();
        while !coloring_queue.is_empty() {
            let n = coloring_queue.pop().unwrap();
            trace!("Assigning color to {}", n);

            let mut ok_colors: LinkedHashSet<usize> =
    ...
    //    fn coloring_queue_heuristic(&self) -> Vec<usize> {
    ...
    /// eliminated
    fn color_heuristic(&self, reg: usize, available_colors: &mut LinkedHashSet<usize>) -> usize {
        ...
        // weight
        let mut candidate_weight: LinkedHashMap<usize, f32> = LinkedHashMap::new();
    ...
    fn rewrite_program(&mut self) {
        ...
        for reg_id in spills.iter() {
            ...
            let mem = self
                .cf
                .frame
                .alloc_slot_for_spilling(ssa_entry.value().clone());

            spilled_mem.insert(*reg_id, mem.clone());
            self.spill_history.insert(*reg_id, mem);
        }

        let scratch_temps = spill_rewrite(&spilled_mem, self.cf);
        for (k, v) in scratch_temps {
            self.spill_scratch_temps.insert(k, v);
        }*/
        panic!("Spills is not yet implemented");
    }

    pub fn spills(&self) -> Vec<usize> {
        let mut spills = vec![];
    ...
    pub fn get_assignments(&self) -> LinkedHashMap<usize, usize> {
    ...
    pub fn get_spill_scratch_temps(&self) -> LinkedHashMap<usize, usize> {
        self.spill_scratch_temps.clone()
    }
}
