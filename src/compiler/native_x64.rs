...
impl Assembler {
    ...
    pub fn push_freg(&mut self, r: XMMRegister) {
        asm::emit_subq_imm_reg(self, 8, RBP);
        asm::movsd_store(self, Mem::Local(0), r);
    }

    pub fn pop_freg(&mut self, r: XMMRegister) {
        asm::movsd_load(self, r, Mem::Local(0));
        asm::emit_addq_imm_reg(self, 8, RBP);
    }
...
pub fn emit_add_mem_reg(buf: &mut Assembler, x64: u8, src: Mem, dest: Register) {
    if dest.msb() != 0 || x64 != 0 {
        emit_rex_mem(buf, x64, dest, &src);
    }
...
pub fn emit_add_reg_mem(buf: &mut Assembler, x64: u8, src: Register, dest: Mem) {
    if src.msb() != 0 || x64 != 0 {
        emit_rex_mem(buf, x64, src, &dest);
    }
    emit_op(buf, 0x01);
    emit_mem(buf, src, &dest);
}

pub fn emit_sub_mem_reg(buf: &mut Assembler, x64: u8, src: Mem, dest: Register) {
    if dest.msb() != 0 || x64 != 0 {
        emit_rex_mem(buf, x64, dest, &src);
    }
...
pub fn emit_sub_reg_mem(buf: &mut Assembler, x64: u8, src: Register, dest: Mem) {
    if src.msb() != 0 || x64 != 0 {
        emit_rex_mem(buf, x64, src, &dest);
    }
    emit_op(buf, 0x29);
...
pub fn emit_callq_target(buf: &mut Assembler, dest: i32) {
    emit_op(buf, 0xe8);
    emit32(buf, dest as u32);
}
