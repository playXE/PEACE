...
impl ModuleTrait for Module {
    ...
    fn get_mut_func<'r>(&'r mut self, fname: String) -> &'r mut Function {
        self.functions.get_mut(&fname).expect(&format!("Func not found {}",fname))
    }

    fn get_func<'r>(&'r self, fname: String) -> &'r Function {
        self.functions.get(&fname).expect(&format!("Func not found {}",fname))
    }
    ...
    fn finish(&mut self) {
        for (name, function) in self.functions.iter_mut() {
            ...
            match func.linkage() {
                ...
                Linkage::DynamicImport(_fname) => unimplemented!(),
                _ => unimplemented!(),
            }
    ...
    fn fix_calls(&mut self) {
        ...
        for (_, fct) in fcts.iter() {
            ...
            for fixup in fct.fixups.iter() {
                ...
                let f: Memory = self.data.get(&fixup.name).expect("Func not found").clone();
...
impl Module {
    pub fn get_data<'r>(&'r mut self, n: String) -> &'r Memory {
        self.data.get(&n).expect(&format!("data `{}` not found",n))
    }
}
